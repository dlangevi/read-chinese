...
func (m *Metadata) UnmarshalJSON(b []byte) error {
	...
	m.Ran = temp.Ran
	m.Dicts = temp.Dicts
	m.PrimaryDict = temp.PrimaryDict

	for name, setting := range temp.UserSettings {
		mSetting, ok := m.UserSettings[name]
		if !ok {
			return errors.New("Mismatch keys")
		}
		mSetting.Value = setting.Value
		m.UserSettings[name] = mSetting
	}
	return nil
}
...
func (settings *UserSettings) GetGroupSettings(groupName string) []UserSetting {
	group := []UserSetting{}
	for _, setting := range userSettings.UserSettings {
