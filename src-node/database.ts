...
interface Store extends Low {
  get: (key: string, val?: any) => any,
  set: (key: string, val: any) => void,
}

let metadataStore: Store;
let knex: KnexType<any, unknown[]>;

export function getKnex() {
  return knex;
}
// This is called and awaited before before anyother code can run
export async function initializeDatabases(userConfigDir:string) {
  const adapter = new JSONFile<MetaData>(
    path.join(userConfigDir, 'metadata.json'),
  );
  ...
  metadataStore.get = (key:string, defaultValue?: any) => {
    const data = metadataStore.data as MetaData;
  ...
  metadataStore.set = (key:string, value: any) => {
    const data = metadataStore.data as MetaData;
    data[key] = value;
    metadataStore.write();
  };
...
export function dbSaveDict(name:string, dictPath:string, type:DictionaryType) {
